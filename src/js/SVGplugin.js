...
class SVGgenerator{ 
	...
	generateDraw(turtle, background, queue, index){
		...
		if(this.turtle_display && this.animation == 1) {
			...
			next = turtle.center(current.from.x, current.from.y)
				...
				}).animate(1000, '-', this.delay).rotate(0, this.distance, 0); //Retour de l'angle à 0
		} else {
	...
	generateRectangle(turtle, background, queue, index){
		...
		//Anime le rectangle
		next = this.rect.animate(1000, '-', this.delay).rotate(current.angle).attr({ width: current.width, height:current.height }).center(current.pos.x, current.pos.y);
		if(current.options.rounded == 0) { this.rect.radius(0)	} else this.rect.radius(10);

		//Positionne la tortue au centre
		if(this.turtle_display && this.animation) turtle = turtle.center(current.pos.x, current.pos.y);
		...
		//Rend l'animation statique si elle n'est pas demandée
		if(!this.animation) this.rect.finish().center(current.pos.x, current.pos.y);
	}
	...
	generateEllipse(turtle, background, queue, index){
		...
		if(this.animation == 1 && current.angle == 0) {
		...
		//Rentre dans le ELSE seulement si une animation est demandée ou non et que l'angle est différent de 0
			...
			next = this.ellipse.animate(1000, '-', this.delay).rotate(current.angle).radius(current.radius_x, current.radius_y);
